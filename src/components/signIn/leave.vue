...
export default {
  ...
  methods: {
    ...
    async getData(type) {
      ...
      let result = await this.$http.post('/api/CrmMobile/GetLeave', this.params)
    ...
    _handleData(result) {
      if (result.Data) {
        result.Data.forEach(item => {
          item.fstartdate = dateFormula.format(new Date(item.fstartdate), 'yyyy-MM-dd hh:mm')
          item.fenddate = dateFormula.format(new Date(item.fenddate), 'yyyy-MM-dd hh:mm')
        })
      }
      return result
    },
    ...
    // 新增
    addDetail() {
      wx.navigateTo({ url: '/pages/taskPages/signIn/leaveDetail/main'})
    },
    // 查看详情
    showDetail(id) {
      wx.navigateTo({ url: '/pages/taskPages/signIn/leaveDetail/main?type=detail&id='+id})
    }
  }
}
</script>
